...
int main(void) {
	...
	N_seq2 = strlen(seq2);
	printf("seq1:%s\nseq2:%s\n\n", seq1, seq2);
	printf("seq1_length=%d\tseq2_length=%d\tpruning=%f (%f%)\tbuffer=%d\n\n", N_seq1,
			N_seq2, pruning, pruning*100,BUFFER);
	//------------------------ Initialising -------------------------
	...
	output = Mind[1][it - 1] - pruning * Mind[1][it - 1];//printf("output=%f\n\n",output);
	//---------------------- Printings -------------------------
	...
	for (n = 1; n < it; n++) {
		...
		if (score <= output) {
			...
			printf("n=%d score=%d, aligning from position %d to %d:\n\n", n,
					score, j, jmax);
			prebuffer = BUFFER;
